...
| **TASK-2.5** | CRITICAL | Production Readiness & Real-World Validation Gate | Hosted readiness, operational reaper scheduling, Flutter seller app shell wiring, and evidence-based Go/No-Go audit | **RESOLVED (CONDITIONAL GO)** | Replaced unmounted Flutter counter app in `main.dart` with `LiveDropSellerApp` and navigation shell; established operational reaper script (`scripts/run-reaper.mjs`) and GitHub Actions cron (`.github/workflows/reaper-cron.yml`); authored hosted Supabase test harness (`scripts/validate-hosted-supabase.mjs`); verified all 14 migrations, 11 triggers, 17 RLS policies, and 356 automated tests. Full verification report in [TASK-2.5-PRODUCTION-READINESS-REPORT](file:///c:/LiveDrop/docs/TASK-2.5-PRODUCTION-READINESS-REPORT.md). |
| **TASK-2.5A** | CRITICAL | Real Staging Deployment, Hosted Runtime Validation & End-to-End Payment Verification | 30-scenario failure injection matrix, secrets hygiene audit, staging reaper verification, multi-connection race tests, and deployment verification | **RESOLVED (CONDITIONAL GO)** | Authored and executed `scripts/test-failure-injections.mjs` with 27 PASS, 3 BLOCKED (tooling/prerequisite), 0 FAIL; verified 356 regression tests, zero service-role leaks in production web chunks, full advance/full-payment/balance lifecycle consistency, and authored comprehensive [TASK-2.5A-STAGING-VALIDATION-REPORT](file:///c:/LiveDrop/docs/TASK-2.5A-STAGING-VALIDATION-REPORT.md). |
| **TASK-2.7** | CRITICAL | Production Vercel Deployment & PostgREST Overload Resolution | Vercel production hosting, seller multi-storefront routing, drop URL sharing in seller app, and PostgREST PGRST203 overload disambiguation on `get_order_by_token` | **RESOLVED** | Deployed `buyer-web` to Vercel production at `https://livedrop.in` (`livedrop-in.vercel.app`); added direct buyer drop URL generation, copy, WhatsApp share, and preview to `seller-app`; resolved PostgreSQL ambiguous function overload in Migration 028 by dropping redundant `get_order_by_token(UUID, UUID)`; verified live order receipt retrieval (HTTP 200) and passed all 391 Vitest and 42 Flutter tests. |
| **TASK-2.8** | HIGH | Multi-Angle Photo Carousel & Realtime Invalidation | Multi-angle photo browsing in buyer web and seller mobile app; dynamic Next.js rendering and Realtime inventory sync on homepage | **RESOLVED** | Migration 029 exposed `image_urls` in `public_products_catalog`; enabled multi-angle hero swipe carousel and thumbnail strips in `buyer-web` and `seller-app` `ProductDetailsScreen`; added `force-dynamic` and Supabase Realtime synchronization to homepage so reservations released in seller app immediately update in buyer browser. |
| **TASK-2.9** | CRITICAL | Advance Paid Remaining Balance Payment Resolution | QR code and UPI amount correctness when buyer returns to pay balance on advance-paid orders | **RESOLVED** | Migration 030 updated `get_order_by_token` to filter out stale advance attempts when `payment_status = 'advance_paid'`, compute `upi_uri` with `balance_due_paisa`, and return the active balance attempt; updated `DirectUpiPaymentView.tsx` to automatically initiate balance payment attempts; added unit test `UI-10` in `persistent-payment-claims-ui.test.tsx`; verified 392 Vitest and 42 Flutter tests pass; deployed to Vercel production. |


---

## 4. Next Action
Deploy staging secrets to GitHub repository, execute `.github/workflows/seller-app-ci.yml` and `.github/workflows/reaper-cron.yml`, and proceed to **TASK-3: Seller Live Operations** (Intake, live drop controls, order Kanban, fulfillment & shipping workflows).
