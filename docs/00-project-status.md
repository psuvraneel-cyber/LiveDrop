...
| **AUDIT-F02** | LOW | Business Logic | `force_release_hold` only releases `pending` holds, not `confirmed` | **RESOLVED** | Documented business rule: confirmed orders require refund-first workflow scheduled for TASK-2.4B. |
| **AUDIT-F06** | LOW | API Completeness | Atomic `mark_order_shipped` RPC absent from schema | **SCHEDULED** | Tracked as core deliverable for TASK-2.4B alongside payment gateway integration. |
