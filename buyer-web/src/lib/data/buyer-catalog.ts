...
/**
 * Known automated test, CI/CD, or non-commercial product title/code patterns.
 */
...
/**
 * Validates whether a product record is legitimate for public storefront display.
 */
export function isLegitimateProduct(product: PublicProductView): boolean {
  if (!product || !product.id || !product.title || !product.code) return false;

  const title = (product.title || '').trim();
  const code = (product.code || '').trim();

  // Test flash codes generated by test runs (e.g. #R1863, #R2754, #R1816)
  if (/^#?R\d{3,}/i.test(code)) return false;

  for (const pattern of TEST_PRODUCT_PATTERNS) {
    if (pattern.test(title)) return false;
  }

  // Reject placeholder test URLs (e.g. example.com)
  if (product.image_url && /example\.com/i.test(product.image_url)) {
    return false;
  }

  // If drop slug or title is attached, verify it does not belong to a test drop
  const dropSlug = (product.drop_slug || '').toLowerCase();
  const dropTitle = (product.drop_title || '').toLowerCase();
  if (dropSlug || dropTitle) {
    for (const pattern of TEST_STORE_PATTERNS) {
      if (pattern.test(dropSlug) || pattern.test(dropTitle)) {
        return false;
      }
    }
  }
...
/**
 * Filters products to expose only legitimate, customer-facing boutique products.
 */
export function filterProductionProducts(products: PublicProductView[]): PublicProductView[] {
  if (!Array.isArray(products)) return [];
  return products.filter(isLegitimateProduct);
}
...
export async function getFeaturedProducts(
  ...
  try {
    ...
    const filtered = filterProductionProducts((data || []) as PublicProductView[]);
...
export async function getAllProducts(
  ...
  try {
    ...
    const maxLimit = options?.limit ? Math.max(options.limit * 3, 80) : 80;
