...
async function runVerification() {
  ...
  try {
    ...
    results.screenshots.push('phase4-home-mobile.png');
    ...
    results.screenshots.push('phase4-shop-mobile.png');
    ...
    results.screenshots.push('phase4-shop-desktop.png');
    ...
    if (await cardGrid.isVisible()) {
      ...
      results.screenshots.push('phase4-product-card-states.png');
    }
    ...
    if (await firstCard.isVisible()) {
      ...
      const detailModal = testPage.locator('[data-testid="product-detail-modal"]');
    ...
    if (await searchInput.isVisible()) {
      ...
      await testPage.waitForTimeout(300);
      const emptyState = testPage.locator('text="No pieces available"');
      const isEmptyVisible = await emptyState.isVisible();
      const exploreCTA = testPage.locator('text="Explore Live Drops"');
    ...
    if (await sareeTab.isVisible()) {
      ...
      results.functionalChecks.push({
        ...
        passed: activeClass?.includes('bg-[#F4F1EA]') || activeClass?.includes('text-black')
      });
    }
    ...
    const dockOcclusion = await testPage.evaluate(() => {
      ...
      // The footer's bottom should extend above or account for dock padding (ld-has-bottom-dock pb-24)
      return {
